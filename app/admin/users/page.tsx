...
export default async function UserDetailPage({ params }: Props) {
  ...
  const user = await prisma.user.findUnique({
    where: { id: userId },
    include: {
      createdTickets: { orderBy:  { createdAt: 'desc' } },
      receivedReports: { orderBy: { createdAt: 'desc' }, include: { creator: true } }
    }
  });
    ...
    <div className="p-8 space-y-8 max-w-6xl mx-auto">
      ...
      <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow border border-gray-200 dark: border-gray-700 flex items-center gap-6">
        ...
        <UserAvatar 
          ...
          name={user.name}
          size="xl"
          className="border-4 border-indigo-100 dark:border-gray-700"
        />
        <div>
